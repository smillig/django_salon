...
class CalendarUserCreationForm(UserCreationForm):
    ...
    email = forms.EmailField(label=gettext_lazy("Email address"), help_text=gettext_lazy("Required"), widget=forms.TextInput(attrs={'class': "form-control", 'placeholder': 'Email Address'}))
    phone_num = forms.CharField(label=gettext_lazy("Phone Number"), required=False, max_length=13, widget=forms.TextInput(attrs={'class': "form-control", 'placeholder': 'Phone Number'}))
    gender = forms.ChoiceField(label=gettext_lazy("Gender"), required=False, choices=GENDER_CHOICES, widget=forms.Select(attrs={'class': "form-control", 'placeholder': 'Gender'}))
    address = forms.CharField(label=gettext_lazy("Address"), required=False, widget=forms.Textarea(attrs={'class': "form-control", 'placeholder': 'Address'}), max_length=255)
    ...
    captcha = CaptchaField(label="Captcha: Confirm you're not a robot.", widget=CaptchaTextInput(attrs={'class': "form-control"}))
    password1 = forms.CharField(label=gettext_lazy("Password"), help_text="<ul><li>Your password can’t be too similar to your other personal information.</li><li>Your password must contain at least 8 characters.</li><li>Your password can’t be a commonly used password.</li><li>Your password can’t be entirely numeric.</li></ul>", widget=forms.PasswordInput(attrs={'class': "form-control", 'placeholder': 'Password'}), max_length=254)
    password2 = forms.CharField(label=gettext_lazy("Confirm Password"),help_text="Enter the same password as before, for verification.", widget=forms.PasswordInput(attrs={'class': "form-control", 'placeholder': 'Confirm Password'}), max_length=254)
